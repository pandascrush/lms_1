...
const CategoryQuizList = () => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:5000/quiz/getquiztype`).then((res) => {
  ...
  useEffect(() => {
    if (selectedCategory && selectedCourseId) {
      ...
      axios
        .get(
          `http://localhost:5000/quiz/questions/${selectedCourseId}/${selectedCategory}`
        )
        .then((res) => {
  ...
  const handleAddQuizzes = () => {
    ...
    axios
      .post("http://localhost:5000/quiz/createquiz", {
